...
DROP TABLE IF EXISTS vnpayproject_dev.bronze.alltransaction_fact;

CREATE TABLE IF NOT EXISTS vnpayproject_dev.bronze.alltransaction_fact 
USING PARQUET
OPTIONS (
  path 'abfss://bronze@vnpayproject.dfs.core.windows.net/alltransactions_fact/'
)
...
DROP TABLE IF EXISTS vnpayproject_dev.bronze.customerlookup;

CREATE TABLE IF NOT EXISTS vnpayproject_dev.bronze.customerlookup
USING PARQUET
OPTIONS (
  path 'abfss://bronze@vnpayproject.dfs.core.windows.net/CustomerLookup_New/'
)
...
DROP TABLE IF EXISTS vnpayproject_dev.bronze.merchant_lookup;

CREATE TABLE IF NOT EXISTS vnpayproject_dev.bronze.merchant_lookup
USING PARQUET
OPTIONS (
  path 'abfss://bronze@vnpayproject.dfs.core.windows.net/merchant_list/'
)
...
DROP TABLE IF EXISTS vnpayproject_dev.bronze.mcc_lookup;

CREATE TABLE IF NOT EXISTS vnpayproject_dev.bronze.mcc_lookup
USING PARQUET
OPTIONS (
  path 'abfss://bronze@vnpayproject.dfs.core.windows.net/mcc_vnpay/'
)
...
SHOW TABLES IN vnpayproject_dev.bronze;

-- **Verify table schemas and properties**
DESCRIBE EXTENDED vnpayproject_dev.bronze.alltransaction_fact;
DESCRIBE EXTENDED vnpayproject_dev.bronze.customerlookup;
DESCRIBE EXTENDED vnpayproject_dev.bronze.merchant_lookup;
DESCRIBE EXTENDED vnpayproject_dev.bronze.mcc_lookup;

-- **Test data accessibility (sample queries)**
-- SELECT COUNT(*) as transaction_count FROM vnpayproject_dev.bronze.alltransaction_fact;
-- SELECT COUNT(*) as customer_count FROM vnpayproject_dev.bronze.customerlookup;
-- SELECT COUNT(*) as merchant_count FROM vnpayproject_dev.bronze.merchant_lookup;
-- SELECT COUNT(*) as mcc_count FROM vnpayproject_dev.bronze.mcc_lookup;

-- **Check for data freshness**
-- SELECT MAX(ingestion_date) as latest_data FROM vnpayproject_dev.bronze.alltransaction_fact;
...
-- %fs ls "abfss://bronze@vnpayproject.dfs.core.windows.net/"
